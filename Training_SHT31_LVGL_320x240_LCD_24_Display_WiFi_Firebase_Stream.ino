...
#define relayDir "node99/control"             //<==========แก้ให้ไม่เหมือนกัน
...
void setup() {
  ...
  Firebase.stream(relayDir, [](FirebaseStream stream) {
    ...
    if (stream.getEvent() == "put") {
      if (stream.getPath() == "/relay1") {
        Tony.digitalWrite(Relay_1, stream.getDataBool());
      }
      if (stream.getPath() == "/relay2") {
        Tony.digitalWrite(Relay_2, stream.getDataBool());
      }
    }
  });
...
void loop() {
  ...
  if (now - lastMeasure > interval){
    ...
    if(!isnan(t) && !isnan(h)){
      ...
      Firebase.setFloat(tempDir, t);
      Serial.print(tempDir);
      Serial.print(" = ");
      Serial.println(t);

      Firebase.setFloat(humiDir, h);
